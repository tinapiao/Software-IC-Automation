...
class bag_serdes_ec__diffamp(Module):
    ...
    param_list = ['lch', 'w_dict', 'th_dict', 'seg_dict', 'dum_info','N','fan_out']
...
#################################LOAD#################################  
        ...
        fg = round_up_to_even(fg/pow(fan_out,N-1))       
        w = w_dict['load']
        th = th_dict['load']
        if N > 1:
         for e in self.instances:
           if e[:5] == 'XLOAD' and int(e[-1]) <= N:
               total_w = fg*pow(fan_out,(N - int(e[-1])))
               self.instances[e].design(wf=w, l=lch, totalM=total_w, intent=th)
...
################################TAIL########################################### 
        ...
        self.instances['XTAILN1'].design(wf=w, l=lch, totalM=fg/2, intent=th)
        self.instances['XTAILP1'].design(wf=w, l=lch, totalM=fg/2, intent=th)
        self.instances['XREF1'].design(wf=w, l=lch, totalM=fg, intent=th)
        fg = round_up_to_even(fg/pow(fan_out,N-1))
        if N > 1:
         for e in self.instances:
            if e[:5] == 'XTAIL' and int(e[-1]) <= N: 
               total_w= fg*pow(fan_out,int(N-int(e[-1])))/2
               self.instances[e].design(wf=w, l=lch, totalM=total_w, intent=th)  
         self.instances['XREF1'].design(wf=w, l=lch, totalM=fg*pow(fan_out,1)/2, intent=th)
###############################IN###############################################       
        fg = seg_dict['in']
        w = w_dict['in']
        th = th_dict['in']
        ...
        if N > 1 :
         for e in self.instances:
           if e[:3] == 'XIN' and int(e[-1]) <= N: 
               total_w= fg*pow(fan_out,int(N-int(e[-1])))
               self.instances[e].design(wf=w, l=lch, totalM=total_w, intent=th)  
