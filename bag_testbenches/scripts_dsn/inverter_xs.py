...
# CHANGE: new fan_out from our calculation
    ...
    finger_p = 100/(rout/40.225) #40.225 ohms for 100 fingers
    '''63ohms for 100finger'''
    seg_pmos = round_up_to_even(finger_p)
    finger_n=100/(rout/15.33)
    seg_nmos=round_up_to_even(finger_n)
    ...
    #Cn = seg_in * (1/3)*pow(10,-15) psh commented out due to not use anywhere in the function
    #R_n = tau/(Cn*8)
    ...
    N = math.ceil(math.log(cload/cin)/math.log(fan_out))
...
def simulate_INVERTER(prj,specs):
    ...
    Outspecs = read_yaml(specs)RTER_layout_new(fan_o
...
def run_main(prj):
    ...
    while not done:
        ...
        seg_pmos = result['seg_pmos']
        seg_nmos = result['seg_nmos']
        ...
        if Swing > Input_Swing*0.9:
            ...
            print("seg_p_psh",seg_pmos)
            print("seg_n_psh",seg_nmos)
