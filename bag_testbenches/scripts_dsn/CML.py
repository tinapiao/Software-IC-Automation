...
def design_amp(amp_specs, Iteration):
    ...
    rout = tau/(cload*8)
    if rout > Rin:
        raise ValueError('Signal can be passed safely without a CML buffer')
...
def CML_Create(prj, specs_fname):
    ...
    result_data,Swing,FT,RT = simulate_CML(prj,specs_fname)
...
def simulate_CML(prj,specs):
    ...
    data_dir = 'demo_data'
    impl_lib = 'A_CML_496'
    gen_cell = 'CML_lch_60n'
    tb_gen_cell = 'CML_tb'
    results_dict = {}
        # setup testbench ADEXL state
    print('setting up testbench')
    tb = prj.configure_testbench('A_CML_496', 'CML_tb')
...
def load_sim_data(specs, dsn_name):
    ...
    print('finish loading data')
def run_main(prj):
    amp_dsn_specs_fname = 'CML_design_files/user_input.yaml'
    amp_char_specs_fname = 'CML_design_files/CML_char.yaml'
    amp_char_specs_out_fname = 'CML_design_files/CML_mod.yaml'
    ...
    while not done:
        ...
        Swing, FT, RT, BW = CML_Create(prj, amp_char_specs_out_fname)
        N = N + 1
        ...
        if Swing > Input_Swing*0.9:
            print("CML Schematic Passed. Now Creating CML Layout")
            print('cml_layout_new_fanout_xs',fan_out)
            print('cml_layout_new_nstage_xs',N_Stage)
            print('cml_layout_new_segload_xs',seg_load)
            print('cml_layout_new_segin_xs',seg_in)
            print('cml_layout_new_segtail_xs',seg_tail)
            CML_layout_new(fan_out, N_Stage, seg_load, seg_in, seg_tail, 60)
        ...
        if N_Stage == 9:
            ...
            print("The best possible CML schematic has been created, Now Creating CML Layout")
            CML_layout_new(fan_out, N_Stage, seg_load, seg_in, seg_tail, 60)
            print("Layout is done")
            print("All designs are done")
            print("Here is the final report:")
            print('User Input is out of range, Requirements can not be met. The best possible CML designs have been created')
            print('FallTime:', FT,"s")
            print('RiseTime:', RT, "s")
            print('3dB_Bandwidth', BW/pow(10,9), "GHz")
            print('Output Swing:', Swing, "V")
