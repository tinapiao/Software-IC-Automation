  ...
  #      if not lvs_passed:
   #         raise ValueError('LVS failed for %s.  Log file: %s' % (dsn_name, lvs_log))
    ...
    def create_dut_layouts(self, lay_params_list, cell_name_list, temp_db):
        ...
        lay_dict1 = lay_params_list[0]['seg_dict1']
        lay_dict2 = lay_params_list[0]['seg_dict2']
        fan_out = lay_params_list[0]['fan_out']
        N = lay_params_list[0]['N']
        #seg_load = lay_dict['load']
        #seg_in = lay_dict['in']
        #seg_tail = lay_dict['tail']
        seg_p1=lay_dict1['pmos1']
        seg_n1=lay_dict1['nmos1']
        seg_p2=lay_dict2['pmos2']
        seg_n2=lay_dict2['nmos2']
        if N < 1:
            ...
            seg_p1 = round_up_to_even(seg_p1 / pow(fan_out, N-1))
            seg_n1 = round_up_to_even(seg_n1 / pow(fan_out, N-1))
            seg_p2 = round_up_to_even(seg_p2 / pow(fan_out, N-1))
            seg_n2 = round_up_to_even(seg_n2 / pow(fan_out, N-1))    
        #sch_list = INVERTER_layout_new(fan_out,N,seg_load,seg_in,seg_tail,60) #oringinally commented, not from psh
        sch_params_list = []
        keys = ['w_dict1', 'w_dict2','N', 'lch', 'fan_out','seg_dict1','seg_dict2','th_dict']
        ...
        sch_params_list[0]['w_dict1']['pmos1'] = str(0.0000002*lay_dict1['pmos1'])
        sch_params_list[0]['w_dict1']['nmos1'] = str(0.0000002*lay_dict1['nmos1'])
        sch_params_list[0]['w_dict2']['pmos2'] = str(0.0000002*lay_dict2['pmos2'])
        sch_params_list[0]['w_dict2']['nmos2'] = str(0.0000002*lay_dict2['nmos2'])
...
def INVERTER_layout(fan_factor, number_fan, number_finger_al, number_finger, number_finger_cm):
    # initialize
...
def INVERTER_layout_single(number_fan, lay_dict):
    # initialize
    laygen = laygo.GridLayoutGenerator(config_file="laygo_config.yaml")
    # template and grid load
    utemplib = laygen.tech + '_invertertemplates'
