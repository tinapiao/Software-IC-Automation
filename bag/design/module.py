...
class Module(DesignMaster, metaclass=abc.ABCMeta):
    ...
    **kwargs :
    ...
    def delete_instance_chain(self, N):
        # type: (str) -> None

        for e in self.instances:
            if e[:3] != 'PIN' and int(e[-1]) > N: self.instances[e] = []
    ...
    def design_dummy_transistors(self, dum_info, inst_name, vdd_name, vss_name, net_map=None):
        ...
        if not dum_info:
            ...
            for idx, ((mos_type, w, lch, th, s_net, d_net), fg) in enumerate(dum_info):
                ...
                self.instances[inst_name][idx].design(wf=w, l=lch, fingers=fg, totalM=fg, intent=th)  #totalM changed to nf
...
class MosModuleBase(Module):
    ...
    **kwargs :
    ...
    def get_params_info(cls):
        # type: () -> Dict[str, str]
        return dict(
            wf='transistor width, in meters or number of fins.',
            l='transistor length, in meters.',
            fingers='transistor number of fingers.',  #totalM changed to nf
            totalM='transistor total number of fingers.',
            intent='transistor threshold flavor.'
        )
##############################must change parameter name
    def design(self, wf=1e-6, l=60e-9, fingers=1, totalM=1, intent='standard'):   #totalM changed to nf
        pass
    ...
    def get_schematic_parameters(self):
        ...
        w = self.params['wf']
        l = self.params['l']
        totalM = self.params['totalM']
        fingers = self.params['fingers']
        ...
        nstr_totm = totalM if isinstance(totalM, str) else '%d' % totalM
        nstr = fingers if isinstance(fingers, str) else '%d' % fingers
        return dict(wf=wstr, l=lstr, fingers=nstr, totalM=nstr_totm) #totalM changed to nf
    ...
    def should_delete_instance(self):
        # type: () -> bool
        return self.params['fingers'] == 0 or self.params['totalM']==0 or self.params['wf'] == 0 or self.params['l'] == 0  #totalM changed to nf
...
class ResPhysicalModuleBase(Module):
    ...
    **kwargs :
    ...
    def design(self, wf=1e-6, l=1e-6, intent='standard'):
        pass
