    ...
    def design_dummy_transistors(self, dum_info, inst_name, vdd_name, vss_name, net_map=None):
        ...
        if not dum_info:
            ...
            for idx, ((mos_type, w, lch, th, s_net, d_net), fg) in enumerate(dum_info):
                ...
                self.instances[inst_name][idx].design(wf=w, l=lch, totalM=fg, intent=th)
    ...
    **kwargs :
    ...
    def get_params_info(cls):
        ...
        return dict(
            ...
            totalM='transistor number of totalM.',
            intent='transistor threshold flavor.'
        )
##############################must change parameter name
    def design(self, wf=1e-6, l=60e-9, totalM=1, intent='standard'):
        pass
    ...
    def get_schematic_parameters(self):
        ...
        return dict(wf=wstr, l=lstr, totalM=nstr)
    ...
    def should_delete_instance(self):
        # type: () -> bool
        return self.params['totalM'] == 0 or self.params['wf'] == 0 or self.params['l'] == 0
